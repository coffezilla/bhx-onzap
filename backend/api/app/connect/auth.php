...
// 
function getAuthorizatedUserData($connection, $userEmail, $currentTimestampClean,  $JWTServerkey, $clientToken, $role = 'ADMIN') {
    ...
    if('Bearer '.$emailValidation === $clientToken) {
        ...
        if($role == 'ADMIN') {
            ...
            cli_email
            FROM clients
            WHERE cli_email = '{$userEmail}' AND cli_status = 1
            ORDER BY cli_id
            DESC
            LIMIT 1") or die ("User Not Found");
        }
        ...
        if (mysqli_num_rows ($queryUsers) > 0) {
            ...
            if($role == 'ADMIN') {
                ...
                $responseData['id'] = $dataUser['cli_id'];
                $responseData['email'] = $dataUser['cli_email'];
            }
