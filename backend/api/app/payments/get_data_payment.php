        ...
        if($AuthUserData['status'] == 1) {
            ...
            if (mysqli_num_rows ($queryPayments) > 0) {
                ...
                while($dataPayment = mysqli_fetch_assoc($queryPayments)) {
                    ...
                    $paySendDate = date_format(date_create_from_format('Y-m-d', $dataPayment["pay_send_date"]), 'd/m/Y');
                    ...
                    // ref
                    $payIdFull = str_pad($paymentId,3, "0", STR_PAD_LEFT);
                    $paymentRef = $dataPayment["cli_short_name"].date('Ym').$payIdFull;
