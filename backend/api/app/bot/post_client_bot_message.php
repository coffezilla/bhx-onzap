...
$keyOption = addslashes(trim($_POST['keyOption']));
$messageId = addslashes(trim($_POST['messageId']));
$answerTo = addslashes(trim($_POST['answerTo']));
...
// verify
$checkers = array($authUserEmail, $message, $keyOption, $answerTo);
...
if($isAuth && $userRole == 'CLIENT') {
    ...
    if($validInputs) {
        ...
        if($AuthUserData['status'] == 1) {
            ...
            // upload photos
            mysqli_query($connection, "INSERT INTO messages VALUES (
            '',
            1,
            '{$message}',
            '{$userId}',
